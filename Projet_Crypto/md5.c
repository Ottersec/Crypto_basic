 ...
 */
void MD5_To_String(unsigned char * texte , char * hash)
{
	int i;
	unsigned char digest[16];

	md5_bis(texte,strlen((char *)texte),digest);

	for(i=0;i<16;i++){
		unsigned char u1 = digest[i] >> 4;
		unsigned char u2 = digest[i] & 0xF;
		sprintf(hash, "%x%x",u1,u2);
		hash+=2;
	}
	*(hash) = '\0';
}
unsigned char *md5_bis (unsigned char *M,  md5_size len, unsigned char *_digest)
{
...
void md5_final (unsigned char *digest, struct md5_ctx *context)
{
	...
	for (i = 0; i < 4; i++){
		...
		digest [i+4] = (unsigned char) ((context->regs.B >> (i*8)) & 0xFF);
		digest [i+8] = (unsigned char) ((context->regs.C >> (i*8)) & 0xFF);
		digest [i+12] = (unsigned char) ((context->regs.D >> (i*8)) & 0xFF);
	}
}
...
static void md5_encode (unsigned char *buffer, struct md5_ctx *context)
{
	...
	II (a, b, c, d, x[12], S41, 0x655b59c3); /* 53 \if*/
